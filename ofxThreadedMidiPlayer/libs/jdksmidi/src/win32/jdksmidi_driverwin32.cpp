...
bool MIDIDriverWin32::StartTimer ( int res )
{
    if ( !timer_open )
    {
        ...
        timer_id = timeSetEvent (
                       res,
                       res,
						/* stubbulon5: WINDOW 10 SDK Didn't like this, so updated the def in  */
                       win32_timer,
                       ( DWORD ) this,
                       TIME_PERIODIC
                   );
...
timeSetEvent(
	...
	_In_ LPTIMECALLBACK fptc,   // win32_timer
	_In_ DWORD_PTR dwUser,
	_In_ UINT fuEvent
);
*/

/* stubbulon5: WINDOW 10 SDK Didn't like this (see above), so updated the def below line WAS:
	(try using this is using older version of windows SDK!)
