...
namespace MusicTheory{
    ...
    public:
        ...
        ScaleData(string scaleId, string scaleName, string scaleIdLower, string origin, vector<string> notes, int numNotes) :
    scaleId(scaleId)
    ,scaleName(scaleName)
    ,scaleIdLower(scaleIdLower)    
    ,origin(origin)
    ,notes(notes)
    ,numNotes(numNotes){ }
        string scaleId;
    ...
    static vector<string> scaleDictionary = getScaleDictionary();
    static vector<ScaleData> scaleDataDictionary;

    static vector<ScaleData> getScaleDataDictionary() {
        if (scaleDataDictionary.size() == 0) {
            // Cache the vector ...
            for (string scale:scaleDictionary) {
                vector<string> scale_parts = ofSplitString(scale, "|");

                string scaleId = (scale_parts.size() > 0) ? scale_parts[0] : "???";
                string scaleName = (scale_parts.size() > 1) ? scale_parts[1] : "???";
                string origin = (scale_parts.size() > 2) ? scale_parts[2] : "???";
                vector<string> notes = (scale_parts.size() > 3) ? ofSplitString(scale_parts[3], ",") : vector<string>();
                if (notes.size() > 0 && notes[notes.size()-1] == ",") notes.pop_back(); // Remove trailing commas entry if any
                int numNotes = notes.size();

                scaleDataDictionary.push_back(
                    ScaleData(scaleId, scaleName, ofToLower(scaleId), origin, notes, numNotes)
                );
            }
        }
        return scaleDataDictionary;
    }    
...
//===================================================================
    ...
    static vector<string> getAllScalesData() {
...
//===================================================================
    ...
    //===================================================================
    #pragma mark - scaleFromDictionary
    //===================================================================
    static shared_ptr<Scale> scaleFromDictionary(string scaleName, NotePtr note) {

        // Expected Tab Seperated format : Scale Id     Scale Name      Origin      Notes       NumNotes
        string scaleIdLower = ofToLower(scaleName);
        vector<ScaleData> sd = getScaleDataDictionary();

        for(ScaleData d:sd) {
            if (d.numNotes >= 4 && d.scaleIdLower == scaleIdLower) {
                vector<string> vec_scaleNotes = d.notes;
                deque<NotePtr> scaleNotes;
                shared_ptr<Scale> scale = Scale::create();
                
                int transpose = Interval::measure(Note::create(vec_scaleNotes[0]), note);

                for(string vec_currNote:vec_scaleNotes) {
                    NotePtr currNote = Note::create(vec_currNote);

                    if (currNote != nullptr) {
                        scaleNotes.push_back(currNote->getTransposed(transpose));
                    } else {
                        return nullptr;
                    }
                }

                scale->name = d.scaleId;
                scale->prettyName = d.scaleName;
...
/*
    static shared_ptr<Scale> scaleFromDictionary(string scaleName, NotePtr note) {
        ...
        // Expected Tab Seperated format : Scale Id     Scale Name      Origin      Notes       NumNotes
        string scaleNameLower = ofToLower(scaleName);
        for(string scaleRow:scaleDictionary) {
            vector<string> vec_scaleRow = ofSplitString(scaleRow, "|", false, true);
            if (vec_scaleRow.size() >= 4 && ofToLower(vec_scaleRow[0]) == scaleNameLower) {
                vector<string> vec_scaleNotes = ofSplitString(vec_scaleRow[3], ",", true, true);
                deque<NotePtr> scaleNotes;
                shared_ptr<Scale> scale = Scale::create();
                ...
                scale->name = vec_scaleRow[0];
                scale->prettyName = vec_scaleRow[1];
                scale->notes = scaleNotes;
                return scale;
            }
        }
...
//===================================================================
    ...
    static string determine(deque<NotePtr> scale){
        if (scale.size() > 0) {
            vector<string> scales = getAllKnownScales();
            shared_ptr<Note> tonic = scale[0];

            for(string fullName:scales) {
                shared_ptr<Scale> scalePtr = create(tonic->name + " " + fullName);

                if (scalePtr != nullptr && scalePtr->notes.size() == scale.size()) {

                    for (int i=0; i<scalePtr->notes.size(); i++) {

                        int diatonicMeasure = Intervals::measure(
                                    Note::create(scalePtr->notes[i]->getDiatonicName()),
                                    Note::create(scale[i]->getDiatonicName())
                         );

                        if (diatonicMeasure != 0) {
                            break;
                        } else if (i == scalePtr->notes.size()-1){
                            string scaleName = scalePtr->getName();
                            return scaleName;
                        }
                    }
                }
...
//===================================================================
    ...
    static string determineFromDictionary(deque<NotePtr> rawScale){
        if (rawScale.size() > 0) {

            // Normalize it to the key of C
            shared_ptr<Note> sourceTonic = rawScale[0];

            int measure = Intervals::measure(
                        Note::create("C"),
                        sourceTonic);

            deque<NotePtr> scale;
            for(shared_ptr<Note> sourceNote:rawScale) {
                measure == 6 ? scale.push_back(sourceNote->getAugmented(measure)) : scale.push_back(sourceNote->getDiminished(measure));
            }

            shared_ptr<Note> tonic = scale[0];

            for(int i=0; i<scaleDictionary.size(); i++) {
               vector<string> scale_parts = ofSplitString(scaleDictionary[i], "|", false, true);

                //ionian	Ionian	Common	C,D,E,F,G,A,B,	7
                string scaleId = "", scaleName = "", origin = "", notesStr = "", numNotes = "";
                ...
                notesStr = (scale_parts.size() > 3) ? scale_parts[3] : "";



                numNotes = (scale_parts.size() > 4) ? scale_parts[4] : "";
                vector<string> notes = ofSplitString(notesStr, ",");
                if (notes[notes.size()-1] == ",") notes.pop_back(); // Remove trailing commas entry if any

                if (notes.size() == scale.size()) {

                    for (int i=0; i<notes.size(); i++) {

                        string n = ofTrim(notes[i]);

                        if (!Note::isValidName(n)) break;
...
//===================================================================
    ...
    static ScaleFunctionLookup ScaleLookup(){
       static ScaleFunctionLookup _scaleLookup ={
            ...
            {"pentatoninicMinorIII",&Scale::getPentatonicMinorIII},
